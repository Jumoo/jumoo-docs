...
### ServerToServerHub
<!--| true/false |-->
Turns the server-to-server SignalR connection (used to report live progress from a target
server back to the source during a push or pull) on or off. On by default. Turning it off just
means coarser progress reporting - it never affects whether a push or pull succeeds. See
[Load Balancing](../../../troubleshooting/load-balancing) for when you might want this off.

### AffinityEnabled
<!--| true/false |-->
Keeps a push or pull's requests pinned to the same target server by remembering and replaying
whatever cookie it sets (an ARR/Application Gateway affinity cookie, for example). On by
default - see [Load Balancing](../../../troubleshooting/load-balancing) for the full picture.

### AffinityCookieNames
<!--| string[] |-->
Cookie names to remember and replay for `AffinityEnabled`. Empty (the default) replays every
cookie the target sets.

### AffinityFailFast
<!--| true/false |-->
Aborts a push or pull the moment its requests stop reaching the same target server, instead of
letting it continue and fail later with a confusing "missing pack folder" error. Off by default.
...
Publisher stores its handlers in a different location. Usually, if you want to turn individual handlers off you can add them to the list of [Disabled Handlers](../../../../uSync/reference/handlers) in the uSync settings. With uSync.Publisher it's largely the same, but the list of disabled handlers is in `Publisher:Handlers:Default:DisabledHandlers`. 

...
Option | note 
-- | -- 
Content | Adds the "Include children" and "Delete missing items" options, will also sync media if required.
...
### Realtime Connector vs Background Connector
uSync ships with two connectors, a realtime connector, and a background connector.

With the realtime connector, the process is driven by your browser asking the server to process each step in real time, so you can see results and feedback along the way. 

With the background connector, the process will run in a background thread and everything will keep going without you having to watch or approve anything. 

Realtime gives immediate feedback, can help you catch errors, and move on to your next step as soon as everything is finished. Background allows you to set and forget for long publishing times, and avoid any timeout issues. 

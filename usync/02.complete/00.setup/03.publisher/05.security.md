...
### Setting Unique Keys Per Server

The sending server will look in the appsettings for `uSync:Publisher:Servers:{serverAlias}:AppId` and `"uSync:Publisher:Servers:{serverAlias}:AppKey` and if they are not there it will use the default values (the 'normal' ones).
...
The target server will need these keys in publisher/settings.

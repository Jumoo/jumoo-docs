...
---
:::warning
As of Umbraco 17, child sites put their files in **usync/v17**. Read our [Folder Changes](/usync/uSync/guides/folderChange) page for more info.
:::
...
Root sites put their files in `/uSync/roots` and ‘child’ sites put their files in `/uSync/v17`.

## Importing with uSync Roots

When importing, uSync will automatically merge `the uSync/roots` and `uSync/v17` trees into one seamless uSync import. This means you get both the changes from the root site and the changes from the child site all in one.

## Saving/Exporting with uSync Roots

When you save on your child site, depending on your settings uSync will either: save a copy of your changes the `uSync/v17` folder **or** it will stop you from making the change.

Files in the `uSync/v17` folder take precedence, so a change on the child site will overwrite a change on the root site.

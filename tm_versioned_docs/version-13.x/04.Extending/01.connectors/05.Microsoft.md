...
---
![Microsoft Translator Page](microsoftpageThrottle.png)
...
By default the throttle is applied for each translation property, but when you have nested properties such as block grid, the throttle only happens once, and if you have a lot of content it might still hit an API limit. 

Enabling throttle per request means the throttle happens every single time a request is made to the API. This is much more often, but also guaranteed to be the same across all items being translated. 

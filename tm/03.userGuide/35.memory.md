...
Once you translate a site with your machine translation connector, you can go to *Translations -> Memory* to see the number of entries and approved entries stored in translation memory.
...
:::info Memory moved in 17.9.0
Before 17.9.0 the Memory screen lived under *Settings -> Translation Manager -> Memory*. It is now in the **Translations** section, alongside the Glossary. If you have upgraded and can't see it, see [Viewing the Glossary and Memory](../getStarted/upgrade#viewing-the-glossary-and-memory).
:::
...
Translation memory is on by default. To switch it off for the whole site, set `MemoryEnabled` to `false` in the `Translation` section of your `appsettings.json`:
...
With memory off, the Memory screen is hidden and connectors stop reading from and writing to it. Anything already stored is left alone, so turning it back on restores what you had.

:::note Who can see it
Seeing the Memory screen needs the **View translation memory** permission as well as access to the Translations section. Administrators are granted it automatically; other user groups have to be given it explicitly. See [Permissions](../getStarted/upgrade#new-permissions-in-1790).
:::
